...
export default function AIAnalysisPanel({ logs, requests, onOpenSettings, onSeekToTime }: AIAnalysisPanelProps) {
  ...
  const handleAnalyze = async (useStreaming = true) => {
    ...
    try {
      ...
      if (data.summary.errorCount === 0 && data.summary.warningCount === 0 && data.summary.networkErrorCount === 0) {
        ...
        <Empty
          ...
          description={
            <Space direction="vertical" size="small">
              ...
              <Text type="secondary">To use AI-powered analysis, you need to configure your OpenAI API key.</Text>
            </Space>
          }
        >
  ...
  return (
    <div className="ai-analysis-panel">
      ...
      <div className="ai-analysis-header">
          ...
          {!analysis && (
            <Space size="small">
              <Text type="secondary" style={{ fontSize: 12 }}>
                {logs.filter((l) => l.level === 'error').length} errors, {logs.filter((l) => l.level === 'warn').length}{' '}
                warnings, {requests.filter((r) => r.response.status >= 400).length} failed requests
              </Text>
            </Space>
          )}
        </div>
        ...
        <Space size="small">
          {analysis && (
            <Button size="small" icon={<BulbOutlined />} onClick={() => handleAnalyze(true)} disabled={analyzing}>
              Re-analyze
            </Button>
          )}
          {onOpenSettings && (
        ...
        {!analyzing && !analysis && !error && (
          <div className="ai-analysis-empty">
            <Space direction="vertical" size="middle" style={{ textAlign: 'center' }}>
              ...
              <Button type="primary" size="large" icon={<ThunderboltOutlined />} onClick={() => handleAnalyze(true)}>
                Start Analysis
              </Button>
            </Space>
          </div>
