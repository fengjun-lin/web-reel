...
function parseFormattedLog(args: any[]): { text: string; styles?: React.CSSProperties }[] {
  ...
  if (!firstArg.includes('%c')) {
    ...
    const allText = args.map((arg) => (typeof arg === 'object' ? JSON.stringify(arg, null, 2) : String(arg))).join(' ');
    return [{ text: allText }];
  }
...
function LogDetail({ log, visible, onClose }: LogDetailProps) {
  ...
  return (
    <Drawer title="Console Log Details" width={720} placement="right" onClose={onClose} open={visible}>
      <div style={{ marginBottom: 16 }}>
        <Descriptions column={1} size="small" bordered>
          <Descriptions.Item label="Level">
            <Text
              strong
              style={{ color: log.level === 'error' ? '#ff4d4f' : log.level === 'warn' ? '#faad14' : undefined }}
            >
...
export default function ConsolePanel({ logs, autoScroll = true, currentTime, onSeekToTime }: ConsolePanelProps) {
  ...
  return (
    <div className="console-panel">
      ...
      <LogDetail log={selectedLog} visible={drawerVisible} onClose={handleDrawerClose} />
    </div>
  );
}
