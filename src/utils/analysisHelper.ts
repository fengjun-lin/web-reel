  ...
  const networkErrors: NetworkErrorInfo[] = recentRequests
    ...
    .map((entry) => ({
      ...
      requestBody: entry.request.postData?.text
        ? truncateText(entry.request.postData.text, 500)
        : undefined,
      responseBody: entry.response.content?.text
        ? truncateText(entry.response.content.text, 500)
        : undefined,
...
**Timestamp (for reference):** ${error.timestamp}ms${
  ...
  if (data.warnings.length > 0) {
    ...
    topWarnings.forEach((warning, index) => {
      ...
      sections.push(`${index + 1}. [${relativeTime}](#seek:${warning.timestamp}) - ${warning.message.slice(0, 180)}${warning.message.length > 180 ? '...' : ''}`)
    })
