...
export async function cleanOldData(
  ...
  const renderEventCount = await db.deleteDataByIndex(
    DB_TABLE_NAME.RENDER_EVENT,
    DB_INDEX_KEY,
    reserveTime
  )
  const responseDataCount = await db.deleteDataByIndex(
  ...
  console.log(
    `[Web-Reel] Total count: ${totalRenderEvent} render events, ${totalResponseData} response data`
  )
  console.log(
    `[Web-Reel] Deleted count: ${renderEventCount} render events, ${responseDataCount} response data`
  )
}
